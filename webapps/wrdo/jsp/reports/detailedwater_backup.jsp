...
window.location.replace("detailedwater_backup.jsp?value1="+vid1+"&&value2="+vid2+"&&value3="+vid3+"&&value4="+vid4+"&&vfdate="+vfdate+"&&vtdate="+vtdate+"&&fhr="+vfhr+"&&fmin="+vfmin+"&&thr="+vthr+"&&tmin="+vtmin);
}
function download_table_as_csv(tblCustomers, separator = ',') {
    ...
    // Download it
    var filename = 'River Gauge Data_' + new Date().toLocaleDateString() + '.csv';
...
query+=" dp.*,soil_moist2,battery,round(cast(scd.wmsl-(solar_rad-scd.mslpoint1) as numeric),3) water_msl,round(cast( scd.paramtime1*power((scd.wmsl-(solar_rad-scd.mslpoint1)),2)+scd.paramtime2*(scd.wmsl-(solar_rad-scd.mslpoint1))+scd.paramtime3 as numeric),4) water_area,round(cast((scd.param4*soil_moist1)+scd.param5 as numeric),3) mean_velocity,round(cast((round(cast(scd.paramtime1*power((scd.wmsl-(solar_rad-scd.mslpoint1)),2)+scd.paramtime2*(scd.wmsl-(solar_rad-scd.mslpoint1))+scd.paramtime3 as numeric),4))*(round(cast((scd.param4*soil_moist1)+scd.param5 as numeric),3))as numeric),4) water_discharge from data_packets as dp,";
