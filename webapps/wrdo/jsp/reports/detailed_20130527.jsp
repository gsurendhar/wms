...
window.location.replace("/wms/jsp/reports/detailed.jsp?value1="+vid1+"&&value2="+vid2+"&&vfdate="+vfdate+"&&vtdate="+vtdate+"&&fhr="+vfhr+"&&fmin="+vfmin+"&&thr="+vthr+"&&tmin="+vtmin);
}
</script>
...
<%
query="select dp.clientid,to_char(packet_date,'mm/dd/yyyy hh24:mi:ss') as date,scd.longitude,scd.latitude,scd.location,scd.state,coalesce((select group_value from parameter where param_id=county),'-') as county,par.group_value,temp,humidity,wind,wind_direction,local_pressure,mslp,rain,hrain from data_packets as dp,(select clientid,longitude,latitude,location,state,district,county from station_configs_det) as scd ,(select param_id,group_value from parameter) as par where packet_date >= to_timestamp('"+vfdate+","+vfhr+":"+vfmin+"','dd/mm/yyyy,hh24:mi:ss') and packet_date < to_timestamp('"+vtdate+","+vthr+":"+vtmin+"','dd/mm/yyyy,hh24:mi:ss') and dp.clientid=scd.clientid and par.param_id=scd.district";
...
<td><%=rs.getString("location")%></td><td><%=rs.getString("date")%></td><td><%=rs.getString("latitude")%></td><td><%=rs.getString("longitude")%></td>
