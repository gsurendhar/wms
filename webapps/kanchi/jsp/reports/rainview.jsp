...
function download_table_as_csv(tblCustomers, separator = ',') {
    ...
    // Download it
    var filename = 'Cummulative Rain_' + new Date().toLocaleDateString() + '.csv';
 ...
 <script type="text/javascript">
        function Export() {
            $("#tblCustomers").table2excel({
				filename: "Cummulative_Rainfall :"+ new Date().toLocaleString().replace(/[\-\:\.]/g, " ") + ".xls"
            });
        }
    </script>
...
<table id="tblCustomers" width="100%" border="1" cellpadding="0" cellspacing="0" id="stretch" >
	<tr><td class="Headings" align="center" colspan=7>Rainfall at Specific Time Report</td></tr>
	...
	<tr><td class="Red-content" align="center" colspan=7><%=vdist+" "+vfdate+" "+vfhr+":"+vfmin %></td></tr>
	<%}
	}%>
	<tr><td>
...
query="select x.clientid,(select group_value from parameter where param_id=district) as district,coalesce((select group_value from parameter where param_id=county),'-') as county,x.location,latitude,longitude,coalesce(y.packet_date,'NA') as packet_date, y.rain from station_configs_det as x left outer join ";
query=query+" (select a.clientid,to_char(a.packet_date,'dd/mm/yyyy hh24:mi') as packet_date,rain from data_packets a, (select clientid,max(packet_date) as packet_date from data_packets where rain is not null and packet_date between ";
...
<% if (rs.getString("rain")==null){ %><td align="right">NA</td><%} else {%><td align="right"><%=rs.getDouble("rain")%></td><%}%>
