...
window.location.replace("stndatareport.jsp?value1="+vid1+"&&value2="+vid2+"&&value3="+vid3+"&&value4="+vid4+"&&vfdate="+vfdate+"&&vtdate="+vtdate+"&&param="+vparam);
}
...
<tr><td class="content style1"><b> Block</b> </td><td>
...
<td class="Blue-content">Wind<br> Direction<br/>(&nbsp;&deg;from&nbsp;north&nbsp;)</td><td class="Blue-content">Humidity<br/>(&nbsp;&permil;&nbsp;)</td><td class="Blue-content">Local Pressure<br>(&nbsp;milibar&nbsp;)</td><td class="Blue-content">MSLP <br>(&nbsp;milibar&nbsp;)</td><td class="Blue-content">Solar&nbspRadiation <br>(&nbsp;watt/meter2&nbsp;)</td></tr>
...
<%} else if ("Wind".equalsIgnoreCase(vparam) ) { %>
	<td class="Blue-content">Wind Speed<br/>(&nbsp;kmph&nbsp;)</td>
<td class="Blue-content">Wind<br>Direction<br/>(&nbsp;&deg;from&nbsp;north&nbsp;)</td></tr>
...
<%} else if ("SolarRadiation".equalsIgnoreCase(vparam) ) { %>
<td class="Blue-content">Solar_Radiation<br>(&nbsp;watt/meter2&nbsp;)</td></tr>


<%}%>
<%
	query="select to_char(packet_date,'DD/MM/YY HH24:MI') as date,round(cast(rain as numeric),2) as rain,round(cast(temp as numeric),2) as temp,round(cast(wind as numeric),2) as wind,round(cast(wind_direction_avg as numeric),1) as wind_direction_avg,round(cast(mslp as numeric),2) as mslp,round(cast(humidity as numeric),2) as humidity,round(cast(solar_rad as numeric),2) as solar_rad,round(cast(local_pressure as numeric),2) as local_pressure from data_packets as a where clientid="+value4+" and packet_type='D' and packet_date between to_date('"+vfdate+"','dd/mm/yyyy') and to_date('"+vtdate+"','dd/mm/yyyy')+ INTERVAL '1 DAY' order by packet_date";
	//out.println(query);
	rs=stmt.executeQuery(query);
...
<td class="readonly"><%=rs.getDouble("wind")%></td><td class="readonly"><%=rs.getDouble("wind_direction_avg")%></td>
...
<% if (rs.getString("rain")==null){ %><td class="readonly">N/A</td><%} else {%><td class="readonly"><%=rs.getDouble("rain")%></td><%}%>
		...
		<% if (rs.getString("wind_direction_avg")==null){ %><td class="readonly">N/A</td><%} else {%><td class="readonly"> <%=rs.getDouble("wind_direction_avg")%></td><%}%>
		...
		<% if (rs.getString("mslp")==null){ %><td class="readonly">N/A</td><%} else {%><td class="readonly"> <%=rs.getDouble("mslp")%></td><%}%>
               <% if (rs.getString("solar_rad")==null){ %><td class="readonly">N/A</td><%} else {%><td class="readonly"> <%=rs.getDouble("solar_rad")%></td><%}%>
...
<%} else if ("SolarRadiation".equalsIgnoreCase(vparam) ) { %>
        <% if (rs.getString("solar_rad")==null){ %><td class="readonly" >N/A</td><%} else {%><td class="readonly"> <%=rs.getDouble("solar_rad")%></td><%}%></tr>
...
ChartUtilities.saveChartAsJPEG(new File("/opt/apache-tomcat-7.0.94/webapps/bws/jsp/reports/chart.jpg"), chart, 900, 500);
