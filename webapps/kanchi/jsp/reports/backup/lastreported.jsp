...
<td class="Blue-content">Wind Packet</td>
<td class="Blue-content">Pressure Packet</td>
</tr>
<%
	
query="select clientid,location,state,district,longitude,latitude,coalesce(to_char(last_packet,'dd/mm/yy hh24:mi'),null,'-') as last_packet, coalesce(to_char(rain_sensors,'dd/mm/yy hh24:mi'),null,'-') as rain_sensors, coalesce(to_char(temp_sensors,'dd/mm/yy hh24:mi'),null,'-') as temp_sensors, coalesce(to_char(wind_sensors,'dd/mm/yy hh24:mi'),null,'-') as wind_sensors, coalesce(to_char(pressure_sensors,'dd/mm/yy hh24:mi'),null,'-') as pressure_sensors from ( select st.clientid,latitude,longitude,location,(select group_value from parameter where param_id=state) as state,(select group_value from parameter where param_id=district) as district,county,last_packet,rain_sensors,temp_sensors,wind_sensors,pressure_sensors from station_configs_det as st left outer join  ( select clientid,max(packet_date) as last_packet, max(case when rain is not null then packet_date end) as rain_sensors, max(case when temp is not null then packet_date end) as temp_sensors, max(case when wind is not null then packet_date end) as wind_sensors,  max(case when local_pressure is not null then packet_date end) as pressure_sensors from data_packets  group by clientid ) as dp on st.clientid=dp.clientid where st.activate!=0";
