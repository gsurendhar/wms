...
window.location.replace("intradayfut.jsp?vgroup="+vgroup+"&&vtrdval="+vtrdval+"&&prcchg="+vchg);
}
</script>
...
query1="select * from (select 'NSE' exchange,i.scripCode scrip_code,concat(i.tradingsymbol,'-',expiry) scrip_name, a.instrument_token instrument ,a.last_price current_price,round(a.price_change/a.open,2) per_change,a.volume_traded qty, ";
...
query1 +=" round((a.last_price-a.open)*100/a.open,2) price_chg_today, round((a.last_price-b.last_price)*100/b.last_price,2) price_chg_delta_per, (a.volume_traded-b.volume_traded) vol_rise_delta,a.close prev_price,a.average_traded_price , ";
//query1 +="r.tottrdqty p_qty, r.tottrdqty prev_traded_qty, round(r.tottrdval/10000000,2) turnover, round((a.volume_traded/r.tottrdqty)*100,2) VOL_RISE_PER, 0 OI_DIF_PER,round(((a.open-r.last)/r.last)*100,2) gap, ";
...
// query1 +=" round(r.TOTTRDVAL/10000000,2) yesterday_turnover,AVG_5D_TOTTRDVAL,";
...
query1 +=" inner join intraday.script_master i on i.scripCode=t.instrument_token and instType='FI'";
...
query1 +=" where 1=1 "+vcon+" order by a.last_traded_time desc )x"+vcon1;
// out.println(query1);
%>
