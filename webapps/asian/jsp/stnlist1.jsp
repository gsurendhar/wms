...
rs=stmt.executeQuery("select a.clientid,(select group_value from parameter where param_id=a.state) as state,(select group_value from parameter where param_id=a.district) as district,coalesce((select group_value from parameter where param_id=a.county),null,'N/A') as county, a.location,a.longitude,a.latitude, coalesce(to_char((last_reported),'MM/DD/YYYY HH24:MI'),null,'-') as packet_date from station_configs_det  as a where activate!=0 and address in ('ARG')  ORDER BY district,county");
%>
<tr><td class="Blue-content" align="center" colspan="2">All Stations List</td></tr>
<%
}
if(value1.equalsIgnoreCase("w")) {
rs=stmt.executeQuery("select a.clientid,(select group_value from parameter where param_id=a.state) as state,(select group_value from parameter where param_id=a.district) as district,coalesce((select group_value from parameter where param_id=a.county),null,'N/A') as county,a.location,a.longitude,a.latitude,coalesce(to_char((last_reported),'MM/DD/YYYY HH24:MI'),null,'-') as packet_date from station_configs_det  as a where activate!=0 and a.clientid  in (select distinct clientid from data_packets where packet_date >= CURRENT_TIMESTAMP - INTERVAL '12 HOUR') and clientid!=15000 and address in ('ARG') order by district,county");
%>
<tr><td class="Blue-content" align="center" colspan="2">Working Stations List</td></tr>
<%
}
if(value1.equalsIgnoreCase("n")) {
rs=stmt.executeQuery("select a.clientid,(select group_value from parameter where param_id=a.state) as state,(select group_value from parameter where param_id=a.district) as district, coalesce((select group_value from parameter where param_id=a.county),null,'N/A') as county,a.location,a.longitude,a.latitude, coalesce(to_char((last_reported),'MM/DD/YYYY HH24:MI'),null,'-') as packet_date from station_configs_det as a where activate!=0 and a.clientid not in (select distinct clientid from data_packets where packet_date >= CURRENT_TIMESTAMP - INTERVAL '12 HOUR'and clientid!=15000) and address in ('ARG') order by district,county,packet_date");
%>
