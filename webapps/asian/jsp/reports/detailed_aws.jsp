...
window.location.replace("detailed_aws.jsp?value1="+vid1+"&&value2="+vid2+"&&vfdate="+vfdate+"&&vtdate="+vtdate+"&&fhr="+vfhr+"&&fmin="+vfmin+"&&thr="+vthr+"&&tmin="+vtmin);
}
...
<table id="tblCustomers" width="100%" border="1" cacellpadding="0" cellspacing="0" id="stretch" > 
	<tr><td class="Headings" align="center" colspan=24 width="80%">All Data Detailed</td></tr>
	<tr><td class="Blue-content" align="center" colspan=24 width="80%"> from <%=vfdate%> <%=vfhr%>:<%=vfmin%> Hrs to <%=vtdate%> <%=vthr%>:<%=vtmin%> Hrs</td></tr>
...
<td colspan="2">Humidity<br>(&#37;)</td>

<td colspan="5">Wind Speed<br>(&nbsp;m/sec&nbsp;)</td>
<td colspan="2">Wind Direction<br>(&nbsp;degrees)</td>
<td rowspan="2">Local Pressure<br>(&nbsp;milibar&nbsp;)</td>
<td rowspan="2">MSLP<br>(&nbsp;milibar&nbsp;)</td> 
...
query+="(case when local_pressure < 0 then null else local_pressure end) as local_pressure,(case when mslp < 0 then null else mslp end) as mslp,rain,hrain,battery,signal_strength ";
//query+=" dp.*, (case when temp < 0 then null else temp end) temp1";
query+=" from data_packets as dp, (select clientid,longitude,latitude,location,state,district,county from station_configs_det where address in ('AWS')) as scd ,";
...
<% if (rs.getString("wind_direction_avg")==null){ %><td align="right">N/A</td><%} else {%><td align="right"><%=rs.getDouble("wind_direction_avg")%></td><%}%>
