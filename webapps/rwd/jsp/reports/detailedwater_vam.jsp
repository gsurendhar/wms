...
<table width="100%" border="0" cacellpadding="0" cellspacing="0" id="stretch" > 
	<tr><td class="Headings" align="center" width="100%">CURRENT HOUR RIVER GAUGE DATA OF VAMSHADAHRA RIVER</td></tr>
	<!--<tr><td class="Blue-content" align="center" width="100%"> from <%=vfdate%> <%=vfhr%>:<%=vfmin%> Hrs to <%=vtdate%> <%=vthr%>:<%=vtmin%> Hrs</td></tr>-->
...
query+=" dp.*,water_level,round(cast(scd.wmsl-(water_level-scd.mslpoint1) as numeric),3) water_msl,round(cast( scd.paramtime1*power((scd.wmsl-(water_level-scd.mslpoint1)),2)+scd.paramtime2*(scd.wmsl-(water_level-scd.mslpoint1))+scd.paramtime3 as numeric),4) water_area,round(cast((scd.param4*water_velocity)+scd.param5 as numeric),3) mean_velocity,round(cast((round(cast(scd.paramtime1*power((scd.wmsl-(water_level-scd.mslpoint1)),2)+scd.paramtime2*(scd.wmsl-(water_level-scd.mslpoint1))+scd.paramtime3 as numeric),4))*(round(cast((scd.param4*water_velocity)+scd.param5 as numeric),3))as numeric),4) water_discharge from data_packets as dp,";
...
query+="(select param_id,group_value from parameter) as par where dp.clientid=6000001 and  packet_type='D' ";
...
query+="and packet_date >=current_timestamp- interval '1 hour'";
...
if(value2.equalsIgnoreCase("-1")) {	query=query+" and state='"+value1+"'";  }
	else{ query=query+" and district=19 ";}
